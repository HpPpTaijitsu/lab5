...
def depth_limited_search(node, depth, current_path, log_files):
    ...
    for child in node.children:
        depth_limited_search(
            child, depth - 1, f"{current_path}/{node.name}", log_files
        )
    ...
    for i in range(1, 12):  # Создаем 11 уровней
        ...
        for j in range(1, 4):
            if j == 1:
                if i % 3 == 0:
                    current.add_child(
                        FileNode(f"file{i}_{j}.log", is_file=True)
                    )
                else:
                    current.add_child(
                        FileNode(f"file{i}_{j}.txt", is_file=True)
                    )
            elif j == 2:
...
if __name__ == "__main__":
    ...
    print(
        "Поиск всех файлов с расширением .log с использованием "
        "алгоритма итеративного углубления"
    )
